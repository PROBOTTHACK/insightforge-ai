...
export function DataTable({ widget }: { widget: TableWidget }) {
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-4 md:col-span-2 xl:col-span-4">
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {widget.rows.map((row, index) => (
              <tr key={index}>
                {widget.columns.map((column) => (
                  <td key={column} className="max-w-64 truncate px-3 py-2 text-slate-700">
                    {String(row[column] ?? "")}
                  </td>
                ))}
              </tr>
